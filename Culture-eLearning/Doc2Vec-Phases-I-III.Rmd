---
title: "Doc2Vec Culture and e-Learning Phases I-III"
...
1. Read in Data Sets and into Single Data Set
```{r}
...
dfRaw  = rbind(dfTrain0, dfTrain1, dfTrain2)
```


2. Data Wrangling: Collapse Subphases into Phases 
```{r}
# Collapse phases
phase1Cols = 5:9
phase2Cols = 10:12
...
dfTrain = data.frame(post=dfRaw$Post, apI= phI, apII=phII, apIII = phIII)
```


3. Data Wrangling : Format Test Data 
```{r}
dfModel = data.frame(NULL)

apCols = c("apIII", "apII", "apI")
uniquePhases=T
...
for (i in 1:length(dfTest$post)) {
  ...
  # Now find the most similar
  phaseCols = c("phI", "phII", "phIII")
  ...
  resultRow = c(i, closeness, dfTest[i,c("apI","apII", "apIII")])
  dfResults = rbind(dfResults, resultRow)
}
dfResults = as.data.frame(dfResults)
colnames(dfResults) = c("postNum", "predicted-I", "predicted-II", "predicted-III", "actual-I", "actual-II", "actual-III")
rownames(dfResults) = NULL
dfResults = apply(dfResults, 2, as.numeric)
#
# Determine correctness
#
predictCols = c("predicted-I", "predicted-II", "predicted-III")
correctCols = c("actual-I", "actual-II", "actual-III")
...
write.csv(dfFinal, "model-results-I-III.csv", row.names=F)
print(accuracy*100)
```
